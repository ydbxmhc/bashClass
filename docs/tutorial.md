...
into=n $c.count           # autogenerated accessor — no .value method needed
printf 'Count: %s\n' "$n"   # Count: 3
```
...
for this object and property. `into=` routes the return value into
the local rather than stdout.

**`$_Self.count "$__Counter_increment_val"`**
Writes the property. The same accessor called with one argument acts
as a setter. Note the `$` before `_Self` — without it, bash would
