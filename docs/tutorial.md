...
The simplest classes can be declared inline and used with only the defaults.

```bash
#!/bin/bash
. boop
boopClass Hero 'has:team,name,wealth'
into=a Hero team=Avengers name='Iron Man' wealth=999999999
_EOL=$' is here!\n' $a.name
```
...
Counter.increment() {
  local _Class="${_Class:-Counter}" _Self="${_Self:-}" # boilerplate for dispatch context
  ...
  (( ++__Counter_increment_val ))              # prefix ++ always nonzero, safe under set -e
...
Reads the `count` property into a local variable via the autogenerated
accessor. `$_Self` expands to the object ID (e.g. `__obj_01`), and
bash appends `.count`, making `__obj_01.count` — the baked wrapper
method for this object and property. `into=` routes the return value
into the variable rather than stdout.

**`$_Self.count "$__Counter_increment_val"`**

Stores the new property value. The same accessor called with one argument
acts as a setter. Note the `$` before `_Self` — without it, bash would
look for a function literally named `_Self.count`, not the object's
baked wrapper.
