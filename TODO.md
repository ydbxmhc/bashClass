...
`has:` properties should work at both class and instance level:

- `ClassName.prop` -- reads/writes the class-level static value
- `$obj.prop` -- reads the instance value if set; falls back to the
  class static if the property was never set on the instance
- `$obj.prop "val"` -- sets the instance value (doesn't touch class)
- `ClassName.prop "val"` -- sets the class static; all instances
  without their own override now see the new default

This is standard class-variable behavior (Ruby, Python, Smalltalk).
The accessor generated by `registerClass`/`backfillMethods` needs to:

1. Check the object descriptor for the property
2. If absent (never set -- NOT empty string, which is a valid value),
   walk up to the class's `_Static` storage
3. The walk could continue up the inheritance chain (class → parent
   class → grandparent...) for inherited defaults

Implementation:
- `registerClass` generates `ClassName.prop()` that reads/writes
  `_Static prop` on the class
- `backfillMethods` generates `$obj.prop()` that checks the object
  descriptor first (`__boop.parse`), falls back to `_Static prop`
  on `_Class` if the property is absent from the descriptor
- "Absent" means the `|prop=...|` segment doesn't exist in the
  descriptor at all -- distinguished from `|prop=|` (empty value)
  via the `+set` check pattern

Use cases:
- Default values: `Greeter.name "World"` -- all Greeters without
  their own name say "Hello, World!"
- Configuration: `Math.precision 20` -- all Math operations use
  20 digits unless overridden per-instance
- Counters: `Greeter.instanceCount` -- class-level shared state
- Constants: `PlayingCard.suits "♠ ♥ ♦ ♣"` -- read-only class data
